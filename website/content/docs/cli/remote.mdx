...
ralph-tui remote add prod server.example.com:7890 --token rtui_abc123...

# Add with default port (7890)
ralph-tui remote add staging staging.local --token rtui_xyz789...

# Add localhost for testing
ralph-tui remote add local localhost --token rtui_test...
```
...
- [`listen`](/docs/cli/listen) - Start a remote listener on a server
...
**Possible causes:**
- Server is not running (`ralph-tui run --listen` or `ralph-tui listen`)
...
**Solution:** Get a fresh token from the server with `ralph-tui listen --rotate-token` and update the remote configuration.
